...
kfc_init(int kthreads, int quantum_us)
{
	...
	*current_tid = 0;
	...
	storage[0].ctid = malloc(sizeof(*storage[0].ctid));
	printf("enqueued main %d, %p\n", *storage[0].ctid, storage[0].ctid);
...
{
  tid_t old = *current_tid;

  printf("Entering kfc_create \n");
  ...
  *ptid = *tid;
  ...
  storage[*ptid].ctid = malloc(sizeof(*storage[*ptid].ctid));
  ...
  *current_tid = *ptid;
  ...
  swapcontext(storage[old].context, storage[*ptid].context);
  return 0;
}
